...
public class XmlUtils {
    ...
    private static ConcurrentMap<String, XStream> xStreamMap = new ConcurrentHashMap();
    ...
    public static String beanToString(Object object) {
        // 初始化处理实例
        XStream xstream = getXStreamBean(object.getClass().getName());
     ...
     *
     * @param xmlStr   xml字符串
     * @param bean     需要转换的对象实例
     * @param rootName xml元素root跟名称
     * @return bean对象
     * @author chenshuaijun
     */
    public static Object xmlStrToBean(String xmlStr, Class<?> bean, String rootName) {
        // 初始化处理实例
        XStream xstream = getXStreamBean(bean.getClass().getName());
        xstream.autodetectAnnotations(true);
        // 设置处理过程中，忽略未知的元素
        xstream.ignoreUnknownElements();
     ...
     * @return
     */
    public static XStream getXStreamBean(String name) {
        XStream xstream = xStreamMap.get(name);
        if (xstream == null) {
            xstream = new XStream(new XppDriver(new XmlFriendlyNameCoder("_-", "_")));
            xStreamMap.put(name, xstream);
        }
        return xstream;
    }
